...
d_tests_sa_filter_op_where
  Tests predicate-based operation constructors: where, where_ctx,
where_not, where_not_ctx.
  Tests the following:
...
)
{
    ...
    // test 2: where_ctx stores predicate and context
    op     = d_filter_where_ctx(pred_greater_than_ctx, &threshold);
    result = d_assert_standalone(
        op.type == D_FILTER_OP_WHERE,
        "where_ctx_type",
        "where_ctx should set D_FILTER_OP_WHERE",
        _counter) && result;

    result = d_assert_standalone(
        op.params.test == pred_greater_than_ctx,
        "where_ctx_predicate",
        "where_ctx should store the predicate function",
        _counter) && result;

    result = d_assert_standalone(
        op.params.context == &threshold,
        "where_ctx_context",
        "where_ctx should store the context pointer",
        _counter) && result;
    ...
    // test 4: where_not_ctx stores predicate and context
    op     = d_filter_where_not_ctx(pred_greater_than_ctx, &threshold);
    result = d_assert_standalone(
        op.type == D_FILTER_OP_WHERE_NOT,
        "where_not_ctx_type",
        "where_not_ctx should set D_FILTER_OP_WHERE_NOT",
        _counter) && result;

    result = d_assert_standalone(
        op.params.context == &threshold,
        "where_not_ctx_context",
        "where_not_ctx should store the context pointer",
        _counter) && result;
